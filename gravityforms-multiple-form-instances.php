...
class Gravity_Forms_Multiple_Form_Instances {
	...
	public function gform_get_form_filter( $form_string, $form ) {
		...
		$strings = array(
			...
			'#gf_' . $form[ 'id' ] . "'"                                      => '#gf_' . $random_id . "'",
			"id='gform_fields_" . $form[ 'id' ]                               => "id='gform_fields_" . $random_id,
			...
			//multi-page forms
			"id='gform_page_" . $form[ 'id' ]                                 => "id='gform_page_" . $random_id,
			"id='gform_previous_button_" . $form[ 'id' ]                      => "id='gform_previous_button_" . $random_id,
			"id='gform_next_button_" . $form[ 'id' ]                          => "id='gform_next_button_" . $random_id,
			'#gform_target_page_number_' . $form[ 'id' ]                      => '#gform_target_page_number_' . $random_id,
			'id=\'gform_target_page_number_' . $form[ 'id' ]                  => 'id=\'gform_target_page_number_' . $random_id,
			'#gform_source_page_number_' . $form[ 'id' ]                      => '#gform_source_page_number_' . $random_id,
			'id=\'gform_source_page_number_' . $form[ 'id' ]                  => 'id=\'gform_source_page_number_' . $random_id,
			"id='gf_page_steps_" . $form[ 'id' ]                              => "id='gf_page_steps_" . $random_id,
			"id='gf_step_" . $form[ 'id' ] . "_"                              => "id='gf_step_" . $random_id . "_"
		);

		$regex_strings = [
			'/(?<=gformInitChosenFields\(\'|,\G)(?:#input_[0-9]+_([0-9]+)(,?))/m' => '#input_' . $random_id . '_$1$2'
		];
		...
		// allow addons & plugins to add additional regex find & replace strings
		$regex_strings = apply_filters( 'gform_multiple_instances_regex_strings', $regex_strings, $form[ 'id' ],
			$random_id );
		...
		// replace all occurences with the new unique ID (using regex)
		foreach ( $regex_strings as $find => $replace ) {
